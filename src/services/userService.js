...
const postPatientBookAppointment = (data) => {
    return axios.post(`http://localhost:8888/api/patient-book-appointment`, data)
}
const postVerifyBookAppointment = (data) => {
    return axios.post(`http://localhost:8888/api/verify-book-appointment`, data)
}

const createNewSpecialty = (data) => {
    return axios.post(`http://localhost:8888/api/create-new-specialty`, data)
}
const getAllSpecialty = () => {
    return axios.get(`http://localhost:8888/api/get-all-specialty`)
}
const getDetailSpecialtyById = (data) => {
    return axios.get(`http://localhost:8888/api/get-detail-specialty-by-id?id=${data.id}&location=${data.location}`)
}

const createNewClinic = (data) => {
    return axios.post(`http://localhost:8888/api/create-new-clinic`, data)
}
const getAllClinic = () => {
    return axios.get(`http://localhost:8888/api/get-all-clinic`)
}
const getDetailClinicById = (data) => {
    return axios.get(`http://localhost:8888/api/get-detail-clinic-by-id?id=${data.id}`)
}

const getAllPatientForDoctor = (data) => {
    return axios.get(`http://localhost:8888/api/get-list-patient-for-doctor?doctorId=${data.doctorId}&date=${data.date}`)
}
const postSendRemedy = (data) => {
    return axios.post(`http://localhost:8888/api/send-remedy`, data)
}
export {
